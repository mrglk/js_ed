const superHeroes = `[
    ...
    },
    {
       ...
       "universe":"Marvel Comics",
       "alter":"Брюс Беннер",
       "occupation":"сверхчеловеческая сила искорость, выносливость, полет",
       "friends":"Мстители",
       "power":"сила, выносливость, бессмертие, быстрая регенерация, мастерство скрытности и боя",
...
const createHero = (data) => {
    ...
    rating.classList.add("slidecontainer");

    const ratingHeader = rating.appendChild(document.createElement("strong"));
    ratingHeader.textContent = "Оценка:"

    const input = rating.appendChild(document.createElement("input"));
    input.classList.add("slider");

    const span = rating.appendChild(document.createElement("span"));
    span.classList.add("slider__span");

    const heroRate = getHeroRating(data.name) 

    input.setAttribute("type", "range");
    input.setAttribute("min", "1");
    input.setAttribute("max", "10");
    input.setAttribute("value", heroRate);
    span.textContent = heroRate;

    input.addEventListener('input', function() {
        span.textContent = input.value;
        const localStorageRating = JSON.parse(localStorage.getItem("heroesRating") || "{}");
        localStorageRating[data.name]=input.value;
        localStorage.setItem("heroesRating", JSON.stringify(localStorageRating))
    });
    
    container.appendChild(div);
}

function getHeroRating(name) {
   const localStorageRating = localStorage.getItem("heroesRating");
   if(!localStorageRating) {
      return 5;
   } else {
      const rating = JSON.parse(localStorageRating);
      if (rating[name]) {
         return rating[name];
      } else {
         return 5;
